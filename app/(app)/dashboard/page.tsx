    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-3">
        ...
        <Card className="flex flex-col justify-center p-6">
          ...
          <div className="mt-1">
            <span className="text-4xl font-bold text-brand-700">{sampleNotes.length}</span>
            ...
            <Card className="flex items-center gap-4 p-4 transition-colors hover:bg-muted/50">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="truncate font-medium text-foreground">{note.topic}</span>
                  <Badge variant={note.format === "kngf" ? "soep" : "soap"}>
                    {note.format === "kngf" ? "SOEP" : "SOAP"}
                  </Badge>
                </div>
                <p className="mt-0.5 truncate text-sm text-muted-foreground">{note.when}</p>
              </div>
            </Card>
          </Link>
        ))}
